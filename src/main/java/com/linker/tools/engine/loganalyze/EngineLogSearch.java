...
public class EngineLogSearch {
    ...
    private static String searchMode = "all";
    ...
    public static void main(String[] args) throws ParseException {
        ...
        if(argsMap.containsKey("-m")){
            searchMode = argsMap.get("-m");

            if(!"s".equals(searchMode) || !"r".equals(searchMode)){
                searchMode = "all";
            }
        }

        EngineLogSearch search = new EngineLogSearch();
        ...
        try {
            ...
            while((line = br.readLine()) != null){
                for(String s : aimStringList){
                    ...
                    }else if("r".equals(searchMode)){
                        if(line.indexOf(RECEIVERMSGFLAG) != -1 && line.indexOf(s) != -1){
                            System.out.println(line);
                            isExist = true;
                            continue;
                        }
                    }else if("s".equals(searchMode)){
                        if(line.indexOf(SENDMSGFLAG) != -1 && line.indexOf(s) != -1){
                            System.out.println(line);
                            isExist = true;
                            continue;
                        }
